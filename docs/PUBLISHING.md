...
5. Создай релиз с тегом `v1.0.0` и приложи **Velune-windows-x64.zip**, **Velune-source.zip** и **SHA256SUMS.txt** из `dist/`. Весь runtime-архив необходим для сохранения сторонних лицензий и исходников LGPL-компонента.

Перед публичным релизом желательно подписать `Velune.exe` по [SIGNING-AND-ANTIVIRUS.md](SIGNING-AND-ANTIVIRUS.md), затем заново создать архив и суммы. Не загружай в репозиторий или Release PFX, пароль от него либо файлы секретов.
